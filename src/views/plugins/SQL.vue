...
const DB = ref(Database.prototype);
const result = ref();
...
<template>
  <var-card :title="'SQLite: ' + DB.path">
    <var-button type="default" block @click="load">load sqlite:test.db</var-button>
    <var-button type="primary" block @click="INSERT({ title: 'title', status: 'status', id: '1' })">INSERT
      INTO
    </var-button>
    <var-button type="warning" block
                @click="UPDATE({ title: 'title2', status: 'status2', id: '1' })">UPDATE
    </var-button>
    <var-button type="success" block @click="select"> select</var-button>
    <var-cell v-for="item of selectResult" :description="JSON.stringify(item)"/>
    <var-button type="danger" block @click="delect(1)"> delect</var-button>
